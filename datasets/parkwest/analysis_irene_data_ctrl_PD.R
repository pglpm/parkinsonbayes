...
learnt <- 'irene_data_ctrl_PD-vrt13_dat116_smp3600'
metadata <- read.csv('meta_irene_data_ctrl_PD.csv',
...
for(percent in c('PFC1_percent', 'SNpc_percent')){
    for(pdgroup in c('PD_subtype_AR_TD', 'PD_subtype_PIGD_TD')){
        ...
        ##
        pdf2(file = paste0(pdgroup, '_', percent))
        par(mfrow = c(3, 3), mai=rep(0.5,4), oma=rep(1,4), mar=c(4,4,0,0))
        for(i in 2:nrow(Y)){
            for(j in 1:(nrow(Y)-1)){
                if(i <= j){
                    plot.new()
